...
void bfdh_f(Bin *bin, FRect *rect, size_t n) {
  ...
  for (size_t i = n; i-- > 0;) {
    ...
    for (size_t j = 0; j < shelf_count; j++) {
      ...
      float waste = bin->w - shelf[j].x - rect[i].w;
      if (waste < 0) {
        continue;
      }
      if (waste < min_waste || min_waste == -1) {
        min_waste = waste;
        best_shelf = j;
      }
    }
...
void bfdh_i(Bin *bin, IRect *rect, size_t n) {
  ...
  for (size_t i = n; i-- > 0;) {
    ...
    for (size_t j = 0; j < shelf_count; j++) {
      ...
      int32_t waste = bin->w - shelf[j].x - rect[i].w;
      if (waste < 0) {
        continue;
      }
      if (waste < min_waste || min_waste == -1) {
